...
export const resetDatabase = async (): Promise<void> => {
  await db.execute(
    sql`truncate table "product_media", "product_specs", "variant_option_values", "product_variants", "product_option_values", "product_options", "products", "spec_attributes", "categories", "suppliers", "media", "password_reset_tokens", "sessions", "users" cascade`,
  )
  outbox.length = 0
}
