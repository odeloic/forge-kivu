...
const { addGalleryItem } = useSuppliers()

const { items, counts, busy, add, retry, remove, clear } = useMediaUpload({
  onReady: async (item) => {
    if (!item.mediaId) return
    await addGalleryItem(props.supplierId, item.mediaId)
    emit('attached')
  },
})
...
<template>
  <UiDialog
    ...
    description="Drop images or choose files. Each one is added to the gallery as it finishes."
  >
